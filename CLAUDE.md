...
- `personal/`: tied to my own internal/company tooling, actively used but not publishable through the public plugin (an installer without that tooling couldn't run them); unlike `misc/`, still linked by `scripts/link-skills.sh`
...
Promoting a skill from a project-local install into this repo (a request shaped like "add `<skill>` from `<repo>` into this repo," or "this project-local skill is ready to share") is the [`promote-project-skill`](./skills/personal/promote-project-skill/SKILL.md) skill. A skill's own prerequisites (a CLI, an MCP namespace, another skill, auth) live on the skill itself, in its `metadata.requires` frontmatter and a `## Prerequisites` section, never a separate repo doc: that's what lets the skill keep working once it's copied standalone into another project. Checking, installing, and live-verifying those prerequisites is the [`verify-skill-setup`](./skills/personal/verify-skill-setup/SKILL.md) skill.
